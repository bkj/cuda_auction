...
__global__ void run_bidding(
	const int num_nodes, float *data, int *person2item, float *bids, int *sbids, float *prices, float auction_eps){
	...
	if(i < num_nodes){
		if(person2item[i] == -1) {
			...
			int   fir_maxObj      = -1;
			int   sec_maxObj      = -1;			
			float fir_maxObjValue = 0.0;
			float sec_maxObjValue = -1000.0;
			float temp_ObjValue   = 0.0;

			fir_maxObj      = 0;
			fir_maxObjValue = data[i * num_nodes + 0] - prices[0];
			
			for(int j = 1; j < num_nodes; j++){
				temp_ObjValue = data[i * num_nodes + j] - prices[j];
				if(temp_ObjValue > fir_maxObjValue){
			
					sec_maxObj      = fir_maxObj;
			...
			double bid = data[i * num_nodes + fir_maxObj] - sec_maxObjValue + auction_eps;
			bids[i * num_nodes + fir_maxObj] = bid;
			atomicMax(sbids + fir_maxObj, 1);
		}
	}
}
...
__global__ void run_assignment(
	const int num_nodes, int *person2item, int *item2person, float *bids, int *sbids, float *prices, int *num_assigned){
